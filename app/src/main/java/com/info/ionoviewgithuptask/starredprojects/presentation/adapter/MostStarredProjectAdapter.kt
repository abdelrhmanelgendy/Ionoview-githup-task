...
class MostStarredProjectAdapter constructor(private val layoutInflater: LayoutInflater) :
    ...
    inner class StarredProjectViewHolder(
        ...
        fun bindDateToViews(position: Int) {
            ...
            holderBinding.gitHupProjectElementTVRepositoryOwnerName.setText(getOwnerName(gitHupProjectItem))

            holderBinding.gitHupProjectElementBtnProjectStarsCount.setText(getStarsCount(gitHupProjectItem))
            holderBinding.gitHupProjectElementBtnProjectIssueCount.setText(getIssueCount(gitHupProjectItem))
            holderBinding.gitHupProjectElementTVRepositoryDescription.setText(gitHupProjectItem.description)
            holderBinding.gitHupProjectElementTVRepositoryCreationDate.setText(getSimpleDate(gitHupProjectItem))

        }

        private fun getStarsCount(gitHupProjectItem: Item)= "Stars ${gitHupProjectItem.stargazers_count}"
        private fun getIssueCount(gitHupProjectItem: Item)= "Issues ${gitHupProjectItem.open_issues_count}"
    ...
    private fun getSimpleDate(gitHupProject: Item) = convertGitHupDateTimeToSimpleDate(gitHupProject.created_at)
